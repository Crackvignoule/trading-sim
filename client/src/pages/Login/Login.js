...
function Login() {
  ...
  const handleLogin = async () => {
    try {
      const response = await fetch('http://localhost:5000/api/login', { // Assurez-vous de mettre à jour l'URL avec votre endpoint
        method: 'POST',
        headers: {
  ...
  const handleRegister = async () => {
    ...
    try {
      const response = await fetch('http://localhost:5000/api/register', { // Assurez-vous de mettre à jour l'URL avec votre endpoint
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
