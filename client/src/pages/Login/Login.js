...
function Login() {
  ...
  const handleLogin = async () => {
    try {
      const response = await fetch(`http://${process.env.REACT_APP_SERVER_URL}/api/login`, { // Assurez-vous de mettre à jour l'URL avec votre endpoint
        method: 'POST',
        headers: {
  ...
  const handleRegister = async () => {
    ...
    try {
      const response = await fetch(`http://${process.env.REACT_APP_SERVER_URL}/api/register`, { // Assurez-vous de mettre à jour l'URL avec votre endpoint
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
